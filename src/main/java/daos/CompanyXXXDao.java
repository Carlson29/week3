...
public class CompanyXXXDao extends Dao implements CompanyDAOInterface {    
    ...
    public Company findBySymbol(String symbol) throws DaoException {
        ...
        try{
            con=getConnection();
            String querry="Select companyName from company where symbol=?";
            ...
            throw new DaoException("CompanyXXXDao findSymbol(): " + e.getMessage());
        } finally {
            try {
                if (rs != null) {
                    rs.close();
    ...
    public List<Company> findCompaniesShareGreaterThan(double share) throws DaoException {
        ...
        try{
            con=getConnection();
            String querry="Select * from company where shareprice>?";
            ps=con.prepareStatement(querry);
